      ...
      __emitCallCallback: function __emitCallCallback(code, config, name, variant, refresh) {
        ...
        if (config.async) {
          ...
          if (config.event) {
            ...
            if (qx.core.Environment.get("qx.promise")) {
              code.push("if(reg.hasListener(this, '", config.event, "Async'))", "reg.fireEventAsync(this, '", config.event, "Async', qx.event.type.Data, [qx.Promise.resolve(computed), old]", ");");
            }
          }
          // Emit code to update the inherited values of child objects
          if (refresh) {
