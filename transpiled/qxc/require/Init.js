...
(function () {
  ...
  qx.Bootstrap.executePendingDefers($$dbClassInfo);qx.Class.define("qxc.require.Init", {
    type: "singleton",
    ...
    statics: {
      ...
      require: function require(modules) {
        return qx.Promise.resolve(_require(modules));
      },
       ...
       */
      fireLoadError: function fireLoadError(err) {
        throw err;
      }
    },
    ...
    members: {
       ...
       */
      addMap: function addMap(id, value) {
        this._map[id] = value;
      },

      addPath: function addPath(moduleId, path) {
       ...
       */
      addResourceManagerPath: function addResourceManagerPath(moduleId, path) {
       ...
       */
      addResourceManagerCss: function addResourceManagerCss(path) {
        var rmPath = qx.util.ResourceManager.getInstance().toUri(path);
        this.addCss(rmPath);
      },
  ...
  qxc.require.Init.$$dbClassInfo = $$dbClassInfo;
})();
...
(function (global, Promise, undef) {
  ...
  var topReq,
      ...
      src,
      subPath,
      bootstrapConfig = requirejs || _require,
    ...
    if (source) {
      eachProp(source, function (value, prop) {
        if (force || !hasProp(target, prop)) {
          if (deepStringMixin && (typeof value === "undefined" ? "undefined" : _typeof(value)) === 'object' && value && !Array.isArray(value) && typeof value !== 'function' && !(value instanceof RegExp)) {
  ...
  function newContext(contextName) {
    ...
    function takeQueue(anonId) {
      ...
      if (anonId) {
        shim = getOwn(_config.shim, anonId) || {};
        main(anonId, shim.deps || [], shim.exportsFn);
      }
    }

    function makeRequire(relName, topLevel) {
      var req = function req(deps, callback, errback, alt) {
        var name, cfg;
    ...
    function waitForDep(depMap, relName, d, i) {
      ...
      _callDep(depMap, relName).then(function (val) {
        d.depFinished(val, i);
    ...
    function makeLoad(id) {
      ...
      load.fromText = function (text, textAlt) {
        ...
        if (hasProp(_config.config, id)) {
          _config.config[plainId] = _config.config[id];
        }
    ...
    load = typeof importScripts === 'function' ? function (map) {
      ...
      script.type = _config.scriptType || 'text/javascript';
      ...
      script.addEventListener('error', function () {
        loadCount -= 1;
        var err,
            pathConfig = getOwn(_config.paths, id);
    ...
    function callPlugin(plugin, map, relName) {
      plugin.load(map.n, makeRequire(relName), makeLoad(map.id), _config);
    }

    _callDep = function callDep(map, relName) {
      var args,
          bundleId,
          name = map.id,
          shim = _config.shim[name];
      ...
      if (name in waiting) {
        ...
        if (map.pr) {
          ...
          if (bundleId = getOwn(bundlesMap, name)) {
            ...
            return _callDep(makeMap(map.pr)).then(function (plugin) {
              // Redo map now that plugin is known to be loaded
              var newMap = map.prn ? map : makeMap(name, relName, true),
                  newId = newMap.id,
                  shim = getOwn(_config.shim, newId);
     ...
     */
    makeMap = function makeMap(name, relName, applyMap) {
      ...
      module: function module(name) {
        return {
          ...
          config: function config() {
            return getOwn(_config.config, name) || {};
          }
        };
      }
    };
    ...
    main = function main(name, deps, factory, errback, relName) {
      if (name) {
      ...
      if (!errback) {
        if (hasProp(_config, 'defaultErrback')) {
          if (_config.defaultErrback) {
            errback = _config.defaultErrback;
          }
        } else {
  ...
  if (topReq.isBrowser && !contexts._.config.skipDataMain) {
    ...
    if (dataMain) {
      ...
      // module ID.
      if ((!bootstrapConfig || !bootstrapConfig.baseUrl) && dataMain.indexOf('!') === -1) {
...
})(undefined, typeof Promise !== 'undefined' ? Promise : undefined);
